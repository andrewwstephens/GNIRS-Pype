...
def writecsv(obsid, date, output='obslog.csv', rawpath=None):
    ...
    for f in files:  # Go through the whole list in case there were interruptions for re-aqcuisitions
        if data[f]['Observation ID'] == obsid:
            ...
            first_science_file = f
    logger.debug('First science file: %s', first_science_file)

    for i in range(files.index(first_science_file)+1, len(files)):  # again, files is reverse sorted
        if data[files[i]]['ACQ'] == 'Y':
            last_acq_file = files[i]
            logger.info('Last acqusition file: %s', last_acq_file)
            break
    ...
    for f in output_data.keys():  # Add new keys for the absolute P and Q offsets:
        ...
        headerinfo[f]['Q'] = headerinfo[f]['QOFFSET'] - headerinfo[last_acq_file]['QOFFSET']
